...
//			printf("Rank: %d  b : %d \n", rank,  atoi(t_str) );
	...
	//	while( 1 )
		...
		//	printf("HERE CHILD \n");
			...
			float* t_x = (float*)malloc( sizeof(float)*n );   // to hold the temporary  values
			...
			for( i =  (rank-1)* div_size ; i <  rank*div_size ; i++ )
			{
				...
				t_x[ i ] = ( b[i - (rank-1)*div_size]  -  sum ) / A[i - (rank-1)*div_size ][i] ;
			}
			
			// send the changed value s of x to root
			MPI_Send( &t_x[ (rank-1)*div_size ], div_size , MPI_FLOAT, 0, 0, MPI_COMM_WORLD );
		}
